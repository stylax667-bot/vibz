...
const tagBtnStyle = (active: boolean, activeColor = '#7F77DD', activeBg = '#EEEDFE', activeText = '#3C3489'): React.CSSProperties => ({
  padding:'6px 14px', borderRadius:20,
  border: active ? `0.5px solid ${activeColor}` : '0.5px solid rgba(0,0,0,0.1)',
    ...
    <div style={{ padding:28, maxWidth:800, display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <div style={card}>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {GENRES.map(g => <button key={g} style={tagBtnStyle(profile.music_genres.includes(g),'#D4537E','#FBEAF0','#4B1528')} onClick={() => toggle('music_genres',g)}>{g}</button>)}
          </div>
        </div>
      </div>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#6b7280', marginBottom:8 }}>Niveau</div>
          <div style={{ display:'flex', gap:8 }}>
            {LEVELS.map(l => (
              <button key={l} onClick={() => setMusicProfile(p => ({ ...p, level: p.level === l ? '' : l }))}
                style={tagBtnStyle(musicProfile.level === l, '#1D9E75', '#E1F5EE', '#085041')}>
                {l}
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#6b7280', marginBottom:8 }}>Influences musicales</div>
          <div style={{ display:'flex', gap:8, marginBottom:8 }}>
            <input
              style={{ ...inp, marginBottom:0, flex:1 }}
              placeholder="Ex: Miles Davis, Radiohead..."
            ...
            <button onClick={() => { addTag('influences', influenceInput); setInfluenceInput('') }}
              style={{ padding:'10px 16px', borderRadius:8, border:'none', background:'#7F77DD', color:'white', cursor:'pointer', fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:13, whiteSpace:'nowrap' }}>
              + Ajouter
            </button>
          </div>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {musicProfile.influences.map(inf => (
              <span key={inf} style={{ padding:'5px 12px', borderRadius:20, background:'#EEEDFE', color:'#3C3489', fontSize:12, fontWeight:700, display:'flex', alignItems:'center', gap:6 }}>
                {inf}
                <span onClick={() => removeTag('influences', inf)} style={{ cursor:'pointer', opacity:0.6, fontSize:14, lineHeight:1 }}>×</span>
              </span>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#6b7280', marginBottom:8 }}>Artistes favoris</div>
          <div style={{ display:'flex', gap:8, marginBottom:8 }}>
            <input
              style={{ ...inp, marginBottom:0, flex:1 }}
              placeholder="Ex: Daft Punk, Adele..."
              value={artistInput}
            ...
            <button onClick={() => { addTag('favorite_artists', artistInput); setArtistInput('') }}
              style={{ padding:'10px 16px', borderRadius:8, border:'none', background:'#D4537E', color:'white', cursor:'pointer', fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:13, whiteSpace:'nowrap' }}>
              + Ajouter
            </button>
          </div>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {musicProfile.favorite_artists.map(a => (
              <span key={a} style={{ padding:'5px 12px', borderRadius:20, background:'#FBEAF0', color:'#4B1528', fontSize:12, fontWeight:700, display:'flex', alignItems:'center', gap:6 }}>
                {a}
                <span onClick={() => removeTag('favorite_artists', a)} style={{ cursor:'pointer', opacity:0.6, fontSize:14, lineHeight:1 }}>×</span>
              </span>
            ))}
          </div>
        </div>
        ...
        <div>
          <div style={{ fontSize:12, fontWeight:700, color:'#6b7280', marginBottom:8 }}>Ma vibe musicale</div>
          <textarea
            style={{ ...inp, height:80, resize:'none', marginBottom:0 }}
            placeholder="Décris ton univers musical, ton ambiance..."
            value={musicProfile.mood_text}
            onChange={e => setMusicProfile(p => ({ ...p, mood_text:e.target.value }))}
          />
        </div>
      </div>
      ...
      <div style={card}>
        ...
        {[{key:'show_socials',label:'Afficher mes réseaux sociaux'},{key:'show_location',label:'Afficher ma ville'}].map(opt => (
          <div key={opt.key} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom:'0.5px solid rgba(0,0,0,0.06)' }}>
            ...
            <button onClick={() => setProfile(p => ({ ...p, [opt.key]:!(p as unknown as Record<string,unknown>)[opt.key] }))}
              style={{ padding:'6px 14px', borderRadius:20, border:'0.5px solid rgba(0,0,0,0.1)', background:(profile as unknown as Record<string,unknown>)[opt.key]?'#E1F5EE':'white', color:(profile as unknown as Record<string,unknown>)[opt.key]?'#085041':'#6b7280', cursor:'pointer', fontFamily:'Syne,sans-serif', fontSize:12, fontWeight:700 }}>
